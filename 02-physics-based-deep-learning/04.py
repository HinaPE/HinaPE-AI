...
real = ti.f32
ti.init(default_fp=real, arch=ti.cpu, flatten_if=True, debug=True)
...
def main():
    ...
    initial_smoke_img = cv2.resize(cv2.imread('front.png'), (n_grid, n_grid))[:, :, 0] / 255.0
    target_img = cv2.resize(cv2.imread('side.png'), (n_grid, n_grid))[:, :, 0] / 255.0
